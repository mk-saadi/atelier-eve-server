...
async function run() {
	try {
		const usersCollection = client.db("techGear").collection("users");
		...
		app.get("/products/category/:category", async (req, res) => {
			try {
				...
				const orQuery = trimmedCategoryFields.map((field) => ({
		...
		app.get("/products/:id", async (req, res) => {
			...
			try {
				...
				const result = await productsCollection.findOne(queryWithObjectId);

				if (!result) {
					const queryWithoutObjectId = { _id: id };
					const fallbackResult = await productsCollection.findOne(queryWithoutObjectId);
...
app.get("/", (req, res) => {
	res.send("Atelier Eve server is running!");
});

app.listen(port, () => {
	console.log(`Atelier Eve server is live on port ${port}`);
});
